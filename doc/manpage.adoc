...
With `--rel-indep` an update of release independent packages can be forced. These are packages that build from a version control system and are not tied to a specific version.
...
Force update / re-adding of release independent packages. These are packages that build from a version control system (VCS) and are not tied to a specific version. Such packages are identified by the corresponding name suffix ("-git", for example). The list of VCS suffixes is maintained in the global configuration file of *repman*.
